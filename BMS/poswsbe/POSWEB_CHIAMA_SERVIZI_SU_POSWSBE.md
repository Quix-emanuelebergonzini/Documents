...
if (pm.request.url.query) {
    pm.request.url.query.map(function (el) {
        if (el.key === "ws_client_pwd") {
            el.value = calcPass(el.value);
        }
        return el;
    });
}
if (pm.request.body) {
    ...
    try {
        ...
        if (body.ws_client_pwd) {
            body.ws_client_pwd = calcPass(body.ws_client_pwd);
...
-- prendere spunto anche da ws_update_invoice_hu --

es: {{url_crm_docker}}/api/v1/om/consumer --> POST
su POSTMAN per passare un body con roba in stringa e roba non in stringa usare il 
pre-request script

var body = {
    "ws_data": {}
}
body["ws_data"] = JSON.stringify({"cod_negozio": "0307090", "data": {
    "pk_consumer": "100233158",
    "brand_logo": "MM",
    "telefono": "+390110110111",
    "nome": "gino",
    "cognome": "pino",
    "email": "ddpopo@yopmail.com"
}})
pm.environment.set('req_body', JSON.stringify(body));

e nel tab body immettere solo
{{req_body}}

così si può fare a meno di scrivere un json con roba in stringa e non....


su BMS cloud si può cercare un utente dentro a clients
from config.ws_server import clients